...
public class TCH_AP_PAUSE_Dao extends HibernateEntityDao<OPCTRANSACTIONLOGTAB, Serializable> {
	...
	public List<OPCTRANSACTIONLOGTAB> getByTxdate(String txDate){
		...
		sql += "FROM OPCTRANSACTIONLOGTAB OPC LEFT JOIN BANK_GROUP BG ON OPC.BANKID = BG.BGBK_ID ";
		//20150324 huangpu 作業類電文的WEBSTEP=01
		sql += "WHERE TXDATE = :txDate AND STAN LIKE '999%' AND PCODE = '1101' AND DATAFIELD = '01' ORDER BY WEBTRACENO";
		try{
			SQLQuery query = getCurrentSession().createSQLQuery(sql);
