...
public class ASYNC_MAC_Dao extends HibernateEntityDao<OPCTRANSACTIONLOGTAB, Serializable> {
	...
	public List<OPCTRANSACTIONLOGTAB> getByTxdate(String txDate, String opbkId){
		List<OPCTRANSACTIONLOGTAB> list = null;
		String sql = "SELECT STAN AS PK_STAN,TXDATE AS PK_TXDATE,TXTIME,PCODE,RSPCODE,FEPPROCESSRESULT,CONCODE,CONTIME,FEPTRACENO,'01' AS IDFIELD,DATAFIELD,INQSTATUS,WEBTRACENO,BANKID, ";
		...
		sql += "WHERE" + (StrUtils.isNotEmpty(txDate)?" TXDATE = :txDate AND":"") + " STAN LIKE '999%' AND PCODE = '1200' ";
		sql += (StrUtils.isNotEmpty(opbkId)?"AND BANKID = '" + opbkId + "' ":"");
		sql += "ORDER BY STAN";
		try{
			SQLQuery query = getCurrentSession().createSQLQuery(sql);
