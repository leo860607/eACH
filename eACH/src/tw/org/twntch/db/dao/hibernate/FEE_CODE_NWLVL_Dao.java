...
import tw.org.twntch.po.FEE_CODE_NWLVL_PK;
import tw.org.twntch.po.SC_COMPANY_PROFILE;
import tw.org.twntch.util.AutoAddScalar;
import tw.org.twntch.util.StrUtils;

public class FEE_CODE_NWLVL_Dao extends HibernateEntityDao<FEE_CODE_NWLVL_Dao, FEE_CODE_NWLVL_PK> {
...
//	}
	...
	public List<FEE_CODE_NWLVL> getdata(String condition, List<String> params) {
		List<FEE_CODE_NWLVL> list = null;
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT * ");
		if (condition.equals("")) {
			sql.append("FROM FEE_CODE_NWLVL  ");
		} else {
			sql.append("FROM FEE_CODE_NWLVL ");
		}
		...
		sql.append(" ORDER BY FEE_LVL_TYPE ");
		try {
			SQLQuery query = getCurrentSession().createSQLQuery(sql.toString());
			for (int i = 0; i < params.size(); i++) {
				query.setParameter(i, params.get(i));
			}
			String ary[] = { "FEE_LVL_TYPE","FEE_DTNO","FEE_LVL_BEG_AMT","FEE_LVL_END_AMT","OUT_BANK_FEE","IN_BANK_FEE","TCH_FEE","SND_BANK_FEE","HANDLECHARGE","WO_BANK_FEE","FEE_ID" };
			AutoAddScalar autoScalar = new AutoAddScalar();
			autoScalar.addScalar(query, FEE_CODE_NWLVL.class, ary);
			query.setResultTransformer(Transformers.aliasToBean(FEE_CODE_NWLVL.class));
			list = query.list();
		} catch (HibernateException e) {
			e.printStackTrace();
		}
...
//	}
	...
	public FEE_CODE_NWLVL findByUK(String fee_uid, String fee_dtno){
		List<FEE_CODE_NWLVL> list = new ArrayList();
		String sql = "select FCL.* ,FC.START_DATE FROM FEE_CODE_NWLVL FCL LEFT JOIN FEE_CODE_NW FC ON FC.FEE_UID = FCL.FEE_UID where FCL.FEE_UID='"+fee_uid+"' and FCL.FEE_DTNO='"+fee_dtno+"'";
		try{
			SQLQuery query = getCurrentSession().createSQLQuery(sql);
			String cols[] = { "START_DATE","FEESEQ","FEE_UID", "FEE_ID", "FEE_LVL_TYPE", "FEE_DTNO", "FEE_LVL_BEG_AMT", "FEE_LVL_END_AMT",
					"OUT_BANK_FEE", "OUT_BANK_FEE_DISC", "IN_BANK_FEE", "IN_BANK_FEE_DISC", "TCH_FEE", "TCH_FEE_DISC",
					"SND_BANK_FEE", "SND_BANK_FEE_DISC", "CDATE", "UDATE", "HANDLECHARGE", "WO_BANK_FEE",
					"WO_BANK_FEE_DISC", "FEE_DESC", "ACTIVE_DESC"};
			AutoAddScalar scalar = new AutoAddScalar();
			scalar.addScalar(query, FEE_CODE_NWLVL.class, cols);
			query.setResultTransformer(Transformers.aliasToBean(FEE_CODE_NWLVL.class));
			list = query.list();
		}catch(HibernateException e){
			e.printStackTrace();
	...
	public List<FEE_CODE_NWLVL> findByFeeUid(String fee_uid){
		List<FEE_CODE_NWLVL> list = new ArrayList();
		String sql ="select FCL.*,COALESCE(TC.TXN_NAME,'') AS TXN_NAME , COALESCE(FC.START_DATE,'') AS START_DATE from FEE_CODE_NWLVL FCL LEFT JOIN TXN_CODE TC ON FCL.FEE_ID = TC.TXN_ID LEFT JOIN FEE_CODE_NW FC ON FCL.FEE_UID = FC.FEE_UID where FCL.FEE_UID='"+fee_uid+"'";
		try{
		SQLQuery query = getCurrentSession().createSQLQuery(sql);
		String cols[] = { "START_DATE","TXN_NAME","FEESEQ","FEE_UID", "FEE_ID", "FEE_LVL_TYPE", "FEE_DTNO", "FEE_LVL_BEG_AMT", "FEE_LVL_END_AMT",
				"OUT_BANK_FEE", "OUT_BANK_FEE_DISC", "IN_BANK_FEE", "IN_BANK_FEE_DISC", "TCH_FEE", "TCH_FEE_DISC",
				"SND_BANK_FEE", "SND_BANK_FEE_DISC", "CDATE", "UDATE", "HANDLECHARGE", "WO_BANK_FEE",
				"WO_BANK_FEE_DISC", "FEE_DESC", "ACTIVE_DESC"};
		AutoAddScalar scalar = new AutoAddScalar();
	...
	public int getLVLRows(String feeUId) {
		List list = null;
		int result = 0;
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT COUNT(*) ");
		sql.append("FROM FEE_CODE_NWLVL WHERE FEE_UID ='" + feeUId + "' ");
