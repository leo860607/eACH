...
public class RPTST_17_Action extends GenericAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		String  yyyymm = "" , outFilename = "_st_17.pdf";
		Map map = null ;
		int export_type = 1;
		System.out.println("RPTST_17_Action is start >> " + ac_key);
...
//			yyyymm = Integer.valueOf(rpt_agent_form.getTW_YEAR())+"/"+rpt_agent_form.getTW_MONTH() ;
			...
			if(ac_key.equalsIgnoreCase("xls")){
				export_type = 2;
				outFilename = "_st_17.xls";
				map = rptst_17_bo.ex_export(rpt_agent_form.getAGENT_COMPANY_ID(), rpt_agent_form.getSND_COMPANY_ID(), yyyymm , rpt_agent_form.getCLEARINGPHASE(), rpt_agent_form.getSerchStrs() , export_type , login_form.getUserData().getUSER_TYPE());
			}else{
				map = rptst_17_bo.export(rpt_agent_form.getAGENT_COMPANY_ID(), rpt_agent_form.getSND_COMPANY_ID(), yyyymm , rpt_agent_form.getCLEARINGPHASE(), rpt_agent_form.getSerchStrs() , export_type ,login_form.getUserData().getUSER_TYPE());
			}
		...
		if(StrUtils.isEmpty(ac_key)){
			String bsDate = rptst_17_bo.getAgent_send_profile_bo().getEachsysstatustab_bo() .getRptBusinessDate();
...
//			rpt_agent_form.setEND_MONTH(getLastMonth(bsDate.substring(4, 6)));
		...
		rpt_agent_form.setTxnIdList(rptst_17_bo.getAgent_send_profile_bo().getAgent_fee_code_bo().getIdList());
		rpt_agent_form.setCompany_IdList(rptst_17_bo.getAgent_send_profile_bo().getAgent_profile_bo().getCompany_Id_ABBR_List());
