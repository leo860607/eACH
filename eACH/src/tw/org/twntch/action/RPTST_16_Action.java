...
public class RPTST_16_Action extends GenericAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		String sbizdate = "" , ebizdate = "" , bizdate = "" , outFilename = "_st_16.pdf";
		Map map = null ;
		int export_type = 1;
		System.out.println("RPTST_16_Action is start >> " + ac_key);
...
//			參加單位端要多加一道檢核
			if(!login_form.getUserData().getUSER_TYPE().equals("A")){
//				bizdate = DateTimeUtils.convertDate(DateTimeUtils.convertDate(rpt_agent_form.getBIZDATE(),"yyyyMMdd","yyyyMMdd"),"yyyyMMdd","營業日期: yyy/MM/dd");
...
//				sbizdate = DateTimeUtils.convertDate(DateTimeUtils.convertDate(rpt_agent_form.getSBIZDATE(),"yyyyMMdd","yyyyMMdd"),"yyyyMMdd","營業日期: yyy/MM/dd");
...
//			export_type = ac_key.equalsIgnoreCase("xls")? 2 : export_type;
//			outFilename = ac_key.equalsIgnoreCase("xls")? "_st_16.xls" : outFilename;
			if(ac_key.equalsIgnoreCase("xls")){
				export_type = 2;
				outFilename = "_st_16.xls";
				map = rptst_16_bo.ex_export(rpt_agent_form.getAGENT_COMPANY_ID(), rpt_agent_form.getSND_COMPANY_ID(), bizdate , rpt_agent_form.getCLEARINGPHASE(), rpt_agent_form.getSerchStrs() , export_type,login_form.getUserData().getUSER_TYPE());
			}else{
				map = rptst_16_bo.export(rpt_agent_form.getAGENT_COMPANY_ID(), rpt_agent_form.getSND_COMPANY_ID(), bizdate , rpt_agent_form.getCLEARINGPHASE(), rpt_agent_form.getSerchStrs() , export_type,login_form.getUserData().getUSER_TYPE());
			}
			BeanUtils.populate(rpt_agent_form, map);
			if(map.get("result").equals("TRUE") && !Arguments.getStringArg("ISWINOPEN").equals("Y")){
//				exportFile2Client(response, map.get("msg").toString(), zDateHandler.getDateNum()+"_st_16.pdf" ,rpt_agent_form.getDow_token() );
		...
		if(StrUtils.isEmpty(ac_key)){
			rpt_agent_form.setBIZDATE(rptst_16_bo.getAgent_send_profile_bo().getEachsysstatustab_bo().getRptBusinessDate()); 
			rpt_agent_form.setSBIZDATE(rpt_agent_form.getBIZDATE()); 
			rpt_agent_form.setEBIZDATE(rpt_agent_form.getBIZDATE()); 
		}
		rpt_agent_form.setTxnIdList(rptst_16_bo.getAgent_send_profile_bo().getAgent_fee_code_bo().getIdList());
		rpt_agent_form.setCompany_IdList(rptst_16_bo.getAgent_send_profile_bo().getAgent_profile_bo().getCompany_Id_ABBR_List());
