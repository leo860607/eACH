...
public class FEE_SEARCH_Action extends GenericAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(ac_key.equalsIgnoreCase("export")){
			Map map = fee_search_bo.qs_ex_export(
			...
			if(map.get("result").equals("TRUE") && !Arguments.getStringArg("ISWINOPEN").equals("Y")){
				exportFile2Client(response, map.get("msg").toString(), zDateHandler.getDateNum()+"_fee_search.xls" ,fee_search_form.getDow_token() );
				return null;
			}else{
				//交易代號清單
