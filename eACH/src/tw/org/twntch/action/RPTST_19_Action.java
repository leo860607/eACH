...
import tw.org.twntch.bo.RPTST_19_BO;
...
public class RPTST_19_Action extends GenericAction {

	private RPTST_19_BO rptst_19_bo ;
	@Override
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		String sbizdate = "" , ebizdate = "" , bizdate = "" , outFilename = "_st_19.pdf";
		Map map = null ;
		int export_type = 1;
		System.out.println("RPTST_19_Action is start >> " + ac_key);
...
//			參加單位端要多加一道檢核
			...
			if(ac_key.equalsIgnoreCase("xls")){
				export_type = 2;
				outFilename = "_st_19.xls";
				map = rptst_19_bo.ex_export(rpt_agent_form.getAGENT_COMPANY_ID(), rpt_agent_form.getSND_COMPANY_ID(), bizdate , rpt_agent_form.getCLEARINGPHASE(), rpt_agent_form.getSerchStrs() , export_type);
			}else{
				map = rptst_19_bo.export(rpt_agent_form.getAGENT_COMPANY_ID(), rpt_agent_form.getSND_COMPANY_ID(), bizdate , rpt_agent_form.getCLEARINGPHASE(), rpt_agent_form.getSerchStrs() , export_type);
			}
		...
		if(StrUtils.isEmpty(ac_key)){
			rpt_agent_form.setBIZDATE(rptst_19_bo.getAgent_send_profile_bo().getEachsysstatustab_bo().getRptBusinessDate()); 
			rpt_agent_form.setSBIZDATE(rpt_agent_form.getBIZDATE()); 
			rpt_agent_form.setEBIZDATE(rpt_agent_form.getBIZDATE()); 
			rpt_agent_form.setTxnIdList(rptst_19_bo.getAgent_send_profile_bo().getAgent_fee_code_bo().getIdList());
			rpt_agent_form.setCompany_IdList(rptst_19_bo.getAgent_send_profile_bo().getAgent_profile_bo().getCompany_Id_ABBR_List());
		}
		target = StrUtils.isEmpty(rpt_agent_form.getTarget())?"":rpt_agent_form.getTarget();
		return mapping.findForward(target);
	}
	public RPTST_19_BO getRptst_19_bo() {
		return rptst_19_bo;
	}
	public void setRptst_19_bo(RPTST_19_BO rptst_19_bo) {
		this.rptst_19_bo = rptst_19_bo;
	}
