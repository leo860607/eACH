...
public class TURNOFF_BO {
	private TURNOFF_Dao turnoff_Dao;
...
//				rtnMap.put(bgbkIdAry[i], JSONUtils.json2map(send(map)));
	...
	public String send(Map<String, String> params){
		...
		try{
			...
			*/
			...
			msgHeader.setStan(turnoff_Dao.getStan());
	...
	public String getDataByStan(Map<String, String> params){
		...
		try{
			list = turnoff_Dao.getByStan(STAN);
			if(list != null){
	...
	public String getDataByDate(Map<String, String> params){
		...
		List<OPCTRANSACTIONLOGTAB> list = null;
		try{
			list = turnoff_Dao.getByTxdate(TXDATE, BGBKID , RSPCODE);
			//查詢成功
			if(list != null){
				result = JSONUtils.toJson(list);
				//寫操作軌跡記錄(成功)
	...
	public TURNOFF_Dao getTurnoff_Dao() {
		return turnoff_Dao;
	}

	public void setTurnoff_Dao(TURNOFF_Dao turnoff_Dao) {
		this.turnoff_Dao = turnoff_Dao;
	}
