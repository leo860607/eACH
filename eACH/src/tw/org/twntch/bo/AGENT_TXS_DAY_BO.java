...
public class AGENT_TXS_DAY_BO {
	...
	public String pageSearch(Map<String, String> params){
		...
		try {
			...
			conditionKey = "[\"SBIZDATE\",\"PCODE\",\"AGENT_COMPANY_ID\",\"SND_COMPANY_ID\",\"CLEARINGPHASE\",\"TG_RESULT\",\"ROWNUMBER\"]";
...
//		sql.append(" , COALESCE(TG_RESULT ,'') TG_RESULT , COALESCE(TRANSACTIONAMOUNT ,'')  NEWTXAMT , COALESCE(CLEARINGPHASE ,'') CLEARINGPHASE , COALESCE(PCODE , '' ) PCODE ");
		...
		sql.append(" FROM  VW_TXNLOG  WHERE 1=1 AND GARBAGEDATA IS NULL ");
		sql.append(sqlpath);
		sql.append(" ) ");
		sql.append(" SELECT A.AGENT_COMPANY_ID  ,A.SND_COMPANY_ID , A.PCODE , A.PCODE ,A.PCODE ||'-'||COALESCE( (SELECT EC.EACH_TXN_NAME FROM EACH_TXN_CODE EC WHERE EC.EACH_TXN_ID = A.PCODE),'') AS PCODE_NAME ,A.COMPANY_ABBR_NAME , A.SND_COMPANY_ABBR_NAME  ");
//		sql.append(" , (CASE  WHEN A.TG_RESULT =  'A' THEN '成功' WHEN A.TG_RESULT = 'R'  OR A.TG_RESULT = 'W' THEN '失敗' ELSE '未完成' END) AS RESULTSTATUS ");
		sql.append(" , (CASE  WHEN A.TG_RESULT =  'A' THEN '成功' WHEN A.TG_RESULT = 'R' THEN '失敗'  WHEN A.TG_RESULT = 'W' THEN '處理中' WHEN A.TG_RESULT = 'U' THEN '與中心不一致' ELSE '未完成' END) AS RESULTSTATUS ");
		sql.append(" , A.FIRECOUNT , A.FIREAMT ");
		sql.append(" FROM ( ");
		sql.append(" 		SELECT ROWNUMBER() OVER() AS ROWNUMBER, T.* FROM ( ");
		sql.append(" 			SELECT AGENT_COMPANY_ID AS AGENT_COMPANY_ID , SND_COMPANY_ID AS SND_COMPANY_ID , PCODE, TG_RESULT ,COMPANY_ABBR_NAME , SND_COMPANY_ABBR_NAME ,SUM(COALESCE(NEWTXAMT,0)) AS FIREAMT , COUNT(*) AS FIRECOUNT FROM TEMP GROUP BY AGENT_COMPANY_ID, SND_COMPANY_ID, PCODE, TG_RESULT ,COMPANY_ABBR_NAME , SND_COMPANY_ABBR_NAME ");
		sql.append(" 		 ) AS T   ");
	...
	public String getCntSQL(String sqlPath ,String sqlPath2  ){
		...
		sql.append(" SELECT COALESCE(AGENT_COMPANY_ID ,'')  AGENT_COMPANY_ID  , COALESCE(SND_COMPANY_ID ,'') SND_COMPANY_ID , COALESCE(RESPONSECODE , '' ) RESPONSECODE  ");
		sql.append(" , COALESCE(COMPANY_ABBR_NAME ,'') COMPANY_ABBR_NAME , COALESCE(SND_COMPANY_ABBR_NAME ,'' ) SND_COMPANY_ABBR_NAME ");
		sql.append(" , COALESCE(TG_RESULT ,'') TG_RESULT ,CAST(COALESCE(TRANSACTIONAMOUNT ,'0') AS INT)   NEWTXAMT , COALESCE(CLEARINGPHASE ,'') CLEARINGPHASE , COALESCE(PCODE , '' ) PCODE ");
		sql.append(" FROM  VW_TXNLOG  WHERE 1=1 AND GARBAGEDATA IS NULL ");
		sql.append(sqlPath);
		sql.append(" ) ");
		sql.append(" SELECT COUNT(*) AS RECORDS , (SELECT COUNT(*) FROM TEMP  ) AS FIRECOUNT, COALESCE((SELECT SUM(COALESCE(NEWTXAMT,0)) FROM TEMP  ),0) AS FIREAMT ");
//		sql.append(" SELECT COUNT(*) AS RECORDS  ");
		sql.append(" FROM (  ");
		sql.append(" 		SELECT  T.*  FROM ( ");
		sql.append(" 			SELECT AGENT_COMPANY_ID AS AGENT_COMPANY_ID , SND_COMPANY_ID AS SND_COMPANY_ID , PCODE, TG_RESULT ,COMPANY_ABBR_NAME , SND_COMPANY_ABBR_NAME  FROM TEMP GROUP BY AGENT_COMPANY_ID, SND_COMPANY_ID, PCODE, TG_RESULT ,COMPANY_ABBR_NAME , SND_COMPANY_ABBR_NAME  ");
		sql.append(" 		) AS T ");
		sql.append(" ) AS A ");
	...
	public Map<String, String> qs_ex_export(Map<String,String> params){
		...
		try{
			...
			String orderSQL = StrUtils.isNotEmpty(sidx)? " ORDER BY "+sidx+" "+sord:"";
			conditionKey = "[\"SBIZDATE\",\"PCODE\",\"AGENT_COMPANY_ID\",\"SND_COMPANY_ID\",\"CLEARINGPHASE\",\"TG_RESULT\"]";
			Map map = getConditionData(params, conditionKey);
			sql = getSQL(map.get("sqlPath").toString(), map.get("sqlPath2").toString() ,orderSQL ) ;
			...
			String outputFilePath = RptUtils.export(RptUtils.COLLECTION,pathType, "agent_txs_day", "agent_txs_day", new HashMap<String, Object>(), list, 2);
