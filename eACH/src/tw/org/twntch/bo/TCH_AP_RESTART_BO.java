...
import tw.org.twntch.db.dao.hibernate.TCH_AP_RESTART_Dao;
...
public class TCH_AP_RESTART_BO {
	private TCH_AP_RESTART_Dao tch_ap_restart_Dao;
	...
	public String preCheck(Map<String, String> params){
		...
		if(StrUtils.isNotEmpty(params.get("signvalue"))){
			...
			//檢查交換所應用系統狀態，必須為暫停狀態才可執行重新啟動
			if(getEachApStatus().equals("1")){
				rtnMap.put("result", "FALSE");
				rtnMap.put("msg", "應用系統已啟動，不可執行重新啟動作業!");
			}
		}
	...
	public String send(Map<String, String> params){
		...
		try{
			...
			*/
			...
			msgHeader.setStan(tch_ap_restart_Dao.getStan());
	...
	public String getDataByStan(Map<String, String> params){
		...
		try{
			list = tch_ap_restart_Dao.getByStan(STAN);
			if(list != null){
	...
	public String getDataByDate(Map<String, String> params){
		...
		List<OPCTRANSACTIONLOGTAB> list = null;
		try{
			list = tch_ap_restart_Dao.getByTxdate(TXDATE);
			if(list != null){
				result = JSONUtils.toJson(list);
			}
		}catch(Exception e){
	...
	public TCH_AP_RESTART_Dao getTch_ap_restart_Dao() {
		return tch_ap_restart_Dao;
	}

	public void setTch_ap_restart_Dao(TCH_AP_RESTART_Dao tch_ap_restart_Dao) {
		this.tch_ap_restart_Dao = tch_ap_restart_Dao;
	}
