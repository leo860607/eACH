...
public class AGENTDL_ST001_BO {
	...
	public String getSQL(String condition){
		...
		sql.append(" ) ");
		sql.append(" ,TEMP2 AS ( ");
		sql.append(" SELECT BIZDATE ,CLEARINGPHASE , AGENT_COMPANY_ID,SND_COMPANY_ID,PCODE,TXID,TG_RESULT,SUM(CNT) CNT,SUM(AMT) AMT ");
		sql.append(" FROM TEMP  GROUP BY BIZDATE ,CLEARINGPHASE ,AGENT_COMPANY_ID,SND_COMPANY_ID,TXID,PCODE,TG_RESULT ");
		sql.append(" ) ");
		...
		sql.append(" ORDER BY AGENT_COMPANY_ID,SND_COMPANY_ID,TXID,PCODE,TG_RESULT  ");
...
//		lastRow = "EOFEA21" + codeUtils.padText("number",6,dataCount);
	...
	public Map<String,Object> getTXT(String date,String agent_company_id , String snd_company_id,String clearingphase ,boolean isWeb){
		...
		//有問題
			...
			dataMap.put("message","查詢代理業者="+agent_company_id+"的交易日統計資料檔過程出現問題");
		}
		return dataMap;
	}
