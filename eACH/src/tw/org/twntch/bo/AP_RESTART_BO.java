...
import tw.org.twntch.db.dao.hibernate.AP_RESTART_Dao;
...
public class AP_RESTART_BO {
	private AP_RESTART_Dao ap_restart_Dao;
	...
	public String preCheck(Map<String, String> params){
		...
		//檢查交換所應用系統狀態，必須為啟動狀態才可執行通知
		if(getEachApStatus().equals("9")){
			rtnMap.put("result", "FALSE");
			rtnMap.put("msg", "應用系統未啟動，不可執行重新啟動通知!");
		}
	...
	public String send(Map<String, String> params){
		...
		try{
			...
			*/
			...
			msgHeader.setPrsCode("1111");
			msgHeader.setStan(ap_restart_Dao.getStan());
			...
			//TODO 測試用，新增假資料
			//ap_restart_Dao.insertTestData(BGBK_ID, msgHeader.getStan());
		}catch(Exception e){
			e.printStackTrace();
		}
		return result;
	...
	public String getDataByStan(Map<String, String> params){
		...
		try{
			list = ap_restart_Dao.getByStan(STAN);
			if(list != null){
	...
	public String getDataByDate(Map<String, String> params){
		...
		List<OPCTRANSACTIONLOGTAB> list = null;
		try{
			list = ap_restart_Dao.getByTxdate(TXDATE, BGBKID ,RSPCODE);
			//查詢成功
			if(list != null){
				result = JSONUtils.toJson(list);
				//寫操作軌跡記錄(成功)
...
//				rtnMap.put(bgbkIdAry[i], JSONUtils.json2map(send(map)));
	...
	public AP_RESTART_Dao getAp_restart_Dao() {
		return ap_restart_Dao;
	}
	public void setAp_restart_Dao(AP_RESTART_Dao ap_restart_Dao) {
		this.ap_restart_Dao = ap_restart_Dao;
	}
