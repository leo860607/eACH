...
<div id="block_body">
			...
			<div id="dataInputTable">
				<html:form styleId="formID" action="/cr_line" method="POST">
					...
					<table>
						<tr>
							<td class="header necessary" style="width: 30%"><span>銀行代號</span></td>
							<td><html:text styleId="BANK_ID" property="BANK_ID"  readonly="true"  styleClass=" lock validate[required] text-input"></html:text></td>
						
							<td><span>銀行名稱</span></td>
							<td ><html:text property="BANK_NAME" readonly="true" styleClass=" lock validate[required] text-input" ></html:text></td>
...
<!-- 								<label class="btn" id="back" onclick="countCR()"><img src="./images/return.png"/>&nbsp;測試</label> -->
							</td>
						</tr>
					</table>
				</html:form>
			...
			function cleanFormII(){
				$.each($('#formID').serializeArray(), function(i, field) {
					if(field.name!="serchStrs"){
						$("#"+field.name).val("");
						if(window.console){console.log(field.name+".val"+$("#"+field.name).val());}
					}
