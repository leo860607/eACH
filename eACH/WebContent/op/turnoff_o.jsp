...
<div id="block_body">
			...
			<div id="opPanel">
				<html:form styleId="formID" action="/turnoff">
							...
							<tr>
								...
								<td style="width: 35%">
									<html:select styleId="BGBK_ID" property="BGBK_ID">
										...
										<logic:present name="turnoff_form" property="bgbkIdList">
											<html:optionsCollection name="turnoff_form" property="bgbkIdList" label="label" value="value"></html:optionsCollection>
										</logic:present>
									</html:select>
								</td>
							</tr>
...
<jsp:include page="/footer.jsp"></jsp:include>
		...
		<script type="text/javascript">
			...
			function alterMsg(){
				var msg = '<bean:write name="turnoff_form" property="msg"/>';
...
// 						startInterval(searchDataByDate, [$("#TXDATE").val()]);
			...
			function send(bgbkId){
				if(preCheck()){
					var rdata = {component:"turnoff_bo",method:"send",BGBK_ID:bgbkId ,action:$("#formID").attr('action')};
			...
			function reSend(chkList){
				var rdata = {component:"turnoff_bo",method:"resend",BGBK_ID_LIST:chkList.toString(),action:$("#formID").attr('action')};
				var msg = fstop.getServerDataExII(uri, rdata, false);
				if(msg != null){
					var failList = [];
					var successList = [];
...
// 							20150421 add by hugo 重送 不管幾家銀行STAN 都一樣
			...
			function preCheck(){
				var rdata = {component:"turnoff_bo",method:"preCheck"};
				var msg = fstop.getServerDataExII(uri, rdata, false);
				if(msg != null){
					if(fstop.isNotEmpty(msg.msg)){
						alert(msg.msg);
			...
			function searchDataByStan(stan){
				...
				searchData("component=turnoff_bo&method=getDataByStan&STAN="+stan);
			}
			
			function searchDataByDate(date, bgbkId , rspcode,serchStrs){
				//if(window.console){console.log("DATE=" + date);}
				searchData("component=turnoff_bo&method=getDataByDate&TXDATE="+date+"&BGBKID="+bgbkId+"&RSPCODE="+rspcode+"&SERCHSTRS="+serchStrs);
			}
