...
<div id="block_body">
			...
			<div id="opPanel">
				<html:form styleId="formID" action="/turnon">
							...
							<tr>
								...
								<td style="width: 35%" >
									<html:select styleId="BGBK_ID" property="BGBK_ID">
										...
										<logic:present name="turnon_form" property="bgbkIdList">
											<html:optionsCollection name="turnon_form" property="bgbkIdList" label="label" value="value"></html:optionsCollection>
										</logic:present>
									</html:select>
								</td>
							</tr>
...
<jsp:include page="/footer.jsp"></jsp:include>
		...
		<script type="text/javascript">
			...
			function alterMsg(){
				var msg = '<bean:write name="turnon_form" property="msg"/>';
						...
						beforeProcessing: function(data, status, xhr){
							...
							<logic:equal name="login_form" property="userData.s_auth_type" value="A">
								for(var rowCount in data){
									...
									data[rowCount].PCODE = data[rowCount].PCODE=="1100"?"開機通知":data[rowCount].PCODE;
								}
							</logic:equal>
...
// 						startInterval(searchDataByDate, [$("#TXDATE").val()]);
			...
			function send(bgbkId){
				if(preCheck()){
					//var rdata = {component:"turnon_bo",method:"send",BGBK_ID:bgbkId};
					var rdata = {component:"turnon_bo",method:"send",BGBK_ID:bgbkId ,action:$("#formID").attr('action')};
			...
			function preCheck(){
				var rdata = {component:"turnon_bo",method:"preCheck"};
				var msg = fstop.getServerDataExII(uri, rdata, false);
				if(msg != null){
					if(fstop.isNotEmpty(msg.msg)){
						alert(msg.msg);
...
// 				var rdata = {component:"turnon_bo",method:"resend",BGBK_ID_LIST:chkList.toString()};
				var rdata = {component:"turnon_bo",method:"resend",BGBK_ID_LIST:chkList.toString(),action:$("#formID").attr('action')};
...
// 							20150421 add by hugo 重送 不管幾家銀行STAN 都一樣
			...
			function searchDataByStan(stan){
				...
				searchData("component=turnon_bo&method=getDataByStan&STAN="+stan);
			}
			
			function searchDataByDate(date, bgbkId , rspcode,serchStrs){
				//if(window.console){console.log("DATE=" + date);}
				searchData("component=turnon_bo&method=getDataByDate&TXDATE="+date+"&BGBKID="+bgbkId+"&RSPCODE="+rspcode+"&SERCHSTRS="+serchStrs);
			}
