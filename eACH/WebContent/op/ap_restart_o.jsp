...
<div id="block_body">
			...
			<div id="opPanel" style="margin-top: 5px">
				<html:form styleId="formID" action="/ap_restart">
							...
							<tr>
								...
								<td>
									<html:select styleId="BGBK_ID" property="BGBK_ID">
										...
										<logic:present name="ap_restart_form" property="bgbkIdList">
											<html:optionsCollection name="ap_restart_form" property="bgbkIdList" label="label" value="value"></html:optionsCollection>
										</logic:present>
									</html:select>
								</td>
							</tr>
							...
							<tr>
								<td colspan="4" style="padding-top: 5px">
									...
									<logic:equal name="login_form" property="userData.s_auth_type" value="A">
										<label class="btn" id="add" name="btn_BGBK_ID" onclick="onPut(this.id)"><img src="./images/send.png"/>&nbsp;送出</label>
									</logic:equal>
								</td>
							</tr>
						</table>
...
<jsp:include page="/footer.jsp"></jsp:include>
		...
		<script type="text/javascript">
			...
			function alterMsg(){
				var msg = '<bean:write name="ap_restart_form" property="msg"/>';
			...
			function initGridOption(){
				gridOption = {
		            	...
		            	colModel: [
							...
							{name:'BANKNAME',index:'BANKNAME',fixed:true,width: 200},
						...
						beforeProcessing: function(data, status, xhr){
							...
							for(var rowCount in data){
								...
								data[rowCount].PCODE = data[rowCount].PCODE=="1111"?"應用系統重新啟動通知":data[rowCount].PCODE;
							}
							</logic:equal>
...
// 						searchDataByDate($("#TXDATE").val(), $("#BGBK_ID").val());
			...
			function send(bgbkId){
				var rdata = {component:"ap_restart_bo",method:"send",BGBK_ID:bgbkId,action:$("#formID").attr('action')};
			...
			function preCheck(){
				var rdata = {component:"ap_restart_bo",method:"preCheck"};
				var msg = fstop.getServerDataExII(uri, rdata, false);
			...
			function reSend(chkList){
				var rdata = {component:"ap_restart_bo",method:"resend",BGBK_ID_LIST:chkList.toString(),action:$("#formID").attr('action')};
				var msg = fstop.getServerDataExII(uri, rdata, false);
				if(msg != null){
					var failList = [];
					var successList = [];
...
// 							20150421 add by hugo 重送 不管幾家銀行STAN 都一樣
			...
			function searchDataByStan(stan){
				...
				searchData("component=ap_restart_bo&method=getDataByStan&STAN="+stan);
			}
			
			function searchDataByDate(date, bgbkId , rspcode,serchStrs){
				//if(window.console){console.log("DATE=" + date);}
// 				searchData("component=ap_restart_bo&method=getDataByDate&TXDATE="+date+"&BGBKID="+bgbkId);
				searchData("component=ap_restart_bo&method=getDataByDate&TXDATE="+date+"&BGBKID="+bgbkId+"&RSPCODE="+rspcode+"&SERCHSTRS="+serchStrs);
			}
