...
<div id="block_body">
			...
			<div id="opPanel">
				<html:form styleId="formID" action="/agent_notice">
...
<%-- 					<html:hidden property="NOTICEID" styleId="NOTICEID" /> --%>
					...
					<fieldset>
							...
							<tr>
								...
								<td style="width:35%">
									<html:select styleId="ABBR_ID" property="ABBR_ID">
										...
										<logic:present name="agent_notice_form" property="abbrIdList">
											<html:optionsCollection name="agent_notice_form" property="abbrIdList" label="label" value="value"></html:optionsCollection>
										</logic:present>
									</html:select>
								</td>
							</tr>
						...
						<div id="msg_dialog" title="訊息內容" style="font-size: 16px">
							操作行：<select onchange="$('#ABBR_ID').val(this.value)">
								<option value="0000000">全部</option>
								<logic:present name="agent_notice_form" property="abbrIdList">
									<logic:iterate id="oBean" name="agent_notice_form" property="abbrIdList">
...
// 						$("#MESSAGE").val("123");
			...
			function alterMsg(){
				var msg = '<bean:write name="agent_notice_form" property="msg"/>';
...
// 						searchDataByDate($("#TXDATE").val(), $("#ABBR_ID").val());
						searchDataByDate($("#TXDATE").val(), $("#ABBR_ID").val() , $("#RSPCODE").val(),$("#serchStrs").val());
					//送出	
					}else if(str == "add"){
						if(confirm("確定送出通知?")){
							blockUI();
// 							20150414 edit by hugo
// 							var stan = send($("#ABBR_ID").val(), $("#MESSAGE").val());
							var stan = send($("#ABBR_ID").val(), $("#MESSAGE").val() , $("#NOTICEID").val());
							if(stan != null){
			...
			function send(abbrId, message , noticeid){
				//判斷是否有中文，有則轉成UTF-8傳送
				if(message.match(/.*[^\u0000-\u007F]+.*/)){
					message = encodeURI(message);
				}
				
				var rdata = {component:"agent_notice_bo",method:"send",ABBR_ID:abbrId,MESSAGE:message , NOTICEID:noticeid,action:$("#formID").attr('action')};
			...
			function reSend(chkList){
				var rdata = {component:"agent_notice_bo",method:"resend",ID_LIST:chkList.toString(),action:$("#formID").attr('action')};
				var msg = fstop.getServerDataExII(uri, rdata, false);
				if(msg != null){
					var failList = [];
					var successList = [];
...
// 							20150421 add by hugo 重送 不管幾家銀行STAN 都一樣
			...
			function searchDataByStan(stan){
				...
				searchData("component=agent_notice_bo&method=getDataByStan&STAN="+stan);
			}
			
			function searchDataByDate(date, abbrId ,rspcode,serchStrs){
				//if(window.console){console.log("DATE=" + date);}
// 				searchData("component=agent_notice_bo&method=getDataByDate&TXDATE="+date+"&ABBRID="+abbrId);
				searchData("component=agent_notice_bo&method=getDataByDate&TXDATE="+date+"&ABBRID="+abbrId+"&RSPCODE="+rspcode+"&SERCHSTRS="+serchStrs);
			}
