...
<div id="block_body">
			...
			<div id="opPanel" style="margin-top: 5px">
				<html:form styleId="formID" action="/tch_turnon">
...
<!-- 										<label class="btn" id="search" onclick="onPut(this.id)"><img src="./images/turnon.png"/>&nbsp;開機</label> -->
										<label class="btn" id="tchTurnOn" onclick="tokenLogin()"><img src="./images/turnon.png"/>&nbsp;開機</label>
									</logic:equal>
...
<jsp:include page="/footer.jsp"></jsp:include>
		...
		<script type="text/javascript">
			...
			function alterMsg(){
				var msg = '<bean:write name="tch_turnon_form" property="msg"/>';
				...
				gridOption = {
						...
						beforeProcessing: function(data, status, xhr){
							...
							for(var rowCount in data){
								...
								data[rowCount].PCODE = data[rowCount].PCODE=="1100"?"交換所開機作業":data[rowCount].PCODE;
							}
						},
			...
			function preCheck(isIkey){
				...
				if(isIkey){
					if($("#signvalue").val() == ""){
						...
						rdata = {component:"tch_turnon_bo",method:"preCheck", signvalue:$("#signvalue").val()};
					}
				}else{
					rdata = {component:"tch_turnon_bo",method:"preCheck"};
				}
...
// 				var rdata = {component:"tch_turnon_bo",method:"send",BGBK_ID:bgbkId};
				var rdata = {component:"tch_turnon_bo",method:"send",BGBK_ID:bgbkId ,action:$("#formID").attr('action')};
				var msg = fstop.getServerDataExII(uri, rdata, false);
				if(msg != null){
					if(fstop.isNotEmpty(msg.msg)){
						alert(msg.msg);
			...
			function searchDataByStan(stan){
				...
				searchData("component=tch_turnon_bo&method=getDataByStan&STAN="+stan);
			}
			
			//此處不使用
			function searchDataByDate(date){
				//if(window.console){console.log("DATE=" + date);}
				searchData("component=tch_turnon_bo&method=getDataByDate&TXDATE="+date);
			}
