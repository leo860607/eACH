...
<div id="block_body">
			...
			<div id="opPanel">
				<html:form styleId="formID" action="/ap_pause">
							...
							<tr>
								...
								<td>
									<html:select styleId="BGBK_ID" property="BGBK_ID">
										...
										<logic:present name="ap_pause_form" property="bgbkIdList">
											<html:optionsCollection name="ap_pause_form" property="bgbkIdList" label="label" value="value"></html:optionsCollection>
										</logic:present>
									</html:select>
								</td>
							</tr>
...
<jsp:include page="/footer.jsp"></jsp:include>
		...
		<script type="text/javascript">
			...
			function alterMsg(){
				var msg = '<bean:write name="ap_pause_form" property="msg"/>';
				...
				gridOption = {
						...
						beforeProcessing: function(data, status, xhr){
							...
							for(var rowCount in data){
								...
								data[rowCount].PCODE = data[rowCount].PCODE=="1110"?"應用系統暫停通知":data[rowCount].PCODE;
							}
							</logic:equal>
...
// 				var rdata = {component:"ap_pause_bo",method:"send",BGBK_ID:bgbkId};
				var rdata = {component:"ap_pause_bo",method:"send",BGBK_ID:bgbkId ,action:$("#formID").attr('action')};
			...
			function preCheck(){
				...
				var rdata = {component:"ap_pause_bo",method:"preCheck"};
				var msg = fstop.getServerDataExII(uri, rdata, false);
				if(msg != null){
					if(fstop.isNotEmpty(msg.msg)){
						alert(msg.msg);
			...
			function reSend(chkList){
				var rdata = {component:"ap_pause_bo",method:"resend",BGBK_ID_LIST:chkList.toString(),action:$("#formID").attr('action')};
				var msg = fstop.getServerDataExII(uri, rdata, false);
				if(msg != null){
					var failList = [];
					var successList = [];
...
// 							20150421 add by hugo 重送 不管幾家銀行STAN 都一樣
			...
			function searchDataByStan(stan){
				...
				searchData("component=ap_pause_bo&method=getDataByStan&STAN="+stan);
			}
			
			function searchDataByDate(date, bgbkId ,rspcode,serchStrs){
				if(window.console){console.log("DATE=" + date);}
// 				searchData("component=ap_pause_bo&method=getDataByDate&TXDATE="+date+"&BGBKID="+bgbkId);
				searchData("component=ap_pause_bo&method=getDataByDate&TXDATE="+date+"&BGBKID="+bgbkId+"&RSPCODE="+rspcode+"&SERCHSTRS="+serchStrs);
			}
