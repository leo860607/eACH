...
--代理業者計費檔下載
SELECT SUBSTR( START_DATE,1,6) YYYYMM ,  COALESCE( (SELECT COMPANY_NO FROM AGENT_PROFILE AP WHERE AP.COMPANY_ID = COMPANY_ID FETCH FIRST 1 ROW ONLY),'') COMPANY_NO ,FEE_ID  ,(CASE WHEN FEE < 0 THEN '-' ELSE '+' END) FEE_SIGN,CAST (FEE AS INTEGER),'' RETCOL
 FROM AGENT_FEE_CODE 
WHERE  SUBSTR( START_DATE,1,6) = '010410'
