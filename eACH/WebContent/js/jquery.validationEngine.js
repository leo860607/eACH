	...
	var methods = {
			...
			for (var i = 0, field_errors = 0; i < rules.length; i++) {
				...
				switch (rules[i]) {
					...
					case "twFuture":
						...
						break;
					case "twDateRange":
						errorMsg = methods._getErrorMessage(form, field, rules[i], rules, i, options, methods._twDateRange);
						break;
					case "mutilIpv4":
					...
					case "userId":
						...
						break;
					case "user_no_required":
						errorMsg = methods._getErrorMessage(form, field, rules[i], rules, i, options, methods._user_no_required);
						if (errorMsg !== undefined) {
							required = true;
						}
						break;
		...
		*/
		...
		_getErrorMessage: function(form, field, rule, rules, i, options, originalValidationMethod) {
			...
			var errorMsg;
			if (rule == "future" || rule == "past" || rule == "maxCheckbox" || rule == "minCheckbox" || rule == "twPast" || rule == "twFuture" || rule == "twDateRange") {
		...
		*/
		...
		/**
		* Check USER_NO required whether
		*
		* @param {jqObject} field
		...
		* @return an error string if validation failed
		*/
		_user_no_required: function(field, rules, i, options) {
			...
			if ("930" == txn_id) {
				...
				if (
					...
					return options.allrules.user_no_required.alertText;
				}
			}
		},

		_checkTXDT: function(field, rules, i, options) {
			if ($('checkTXTD').val = '') {
				return options.allrules.checkTXDT.alertText;
			}
		},
		...
		*/
		...
		_twDateRange: function(form, field, rules, i, options) {
			var startdateStr = $(form.find("*[name='" + rules[i + 1].replace(/^#+/, '') + "']")).val();
			var enddateStr = $(form.find("*[name='" + rules[i + 2].replace(/^#+/, '') + "']")).val();
			var endInput = $(form.find("*[name='" + rules[i + 2].replace(/^#+/, '') + "']"))
			if (endInput.is(":disabled") || enddateStr == '') return;
			if (undefined != startdateStr && undefined != enddateStr) {
				var startday = new Date(startdateStr.substring(0, 4), startdateStr.substring(4, 6) - 1, startdateStr.substring(6, 8));
				var endday = new Date(enddateStr.substring(0, 4), enddateStr.substring(4, 6) - 1, enddateStr.substring(6, 8));
				var limitday = new Date(enddateStr.substring(0, 4), enddateStr.substring(4, 6) - 1, enddateStr.substring(6, 8));
				limitday.setDate(limitday.getDate() - rules[i + 3] + 1);
				if (startday < limitday) {
					var rule = options.allrules.twDateRange;
					if (rule.alertText2) return rule.alertText + rules[i + 3] + rule.alertText2;
					return rule.alertText + methods._parseWestDate(methods._dateToString(fdate));
				}
			}
		},
		...
		_userId: function(field, rules, i, options) {
			...
			for (var index in ary) {
				//				20181219 edit by hugo 依建利需求 左方區塊為數字，右方區塊改為英數字混合 長度不變
				...
				if (index == 0) {
					...
					//					20181219 edit by hugo 依建利需求 左方區塊為數字，右方區塊改為英數字混合 長度不變
					if (!ary[index].match(/^[\-\+]?((([0-9]{1,3})([,][0-9]{3})*)|([0-9]+))?([\.]([0-9]+))?$/)) {
						return options.allrules.userId.alertText + "用戶代號格式不符，'-'左方區塊必須是數字，參考範例0100000-01";
					}
				}
				if (index == 1) {
					...
					//					20181219 edit by hugo 依建利需求 左方區塊為數字，右方區塊改為英數字混合 長度不變
					if (!ary[index].match(/^[a-zA-Z0-9]+$/)) {
						return options.allrules.userId.alertText + "用戶代號格式不符，'-'右方區塊必須是英數字，參考範例0100000-01或0100000-a1";
					}
				}
			}
		},
