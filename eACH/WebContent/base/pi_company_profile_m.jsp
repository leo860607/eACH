			...
			<div id="opPanel">
				<html:form styleId="formID" action="/pi_company_profile" method="POST" >
					...
					<fieldset>
						...
						<table id="search_tab">
							<tr>
								<td class="header" style="width: 18%">收費業者統一編號</td>
								<td style="width: 30%">
									<html:text styleId="PI_COMPANY_ID" property="PI_COMPANY_ID" size="10" maxlength="10"></html:text>
								</td>
...
<%-- 								<td><html:text styleId="SND_COMPANY_ID" property="SND_COMPANY_ID" size="15" maxlength="10"></html:text></td> --%>
							...
							<tr>
		                        ...
		                        <td colspan="3"><html:text styleId="PI_COMPANY_NAME" property="PI_COMPANY_NAME" maxlength="65" style="width: 100%"></html:text></td>
							</tr>
							<tr>
		...
		<script type="text/javascript">
			...
			function initGrid(){
				gridOption = {
		            	...
		            	rowNum: 10000,
		            	hiddengrid: true,
		            	colNames:['檢視明細','收費業者統一編號','收費業者簡稱','收費業者名稱','交易代號','繳費類別代號','約定授權','晶片金融卡','本人帳戶繳本人帳單','啟用日期','停用日期','備註'],
		            	colModel: [
		            	    {name:'BTN', fixed: true, align: 'center', width:60, sortable: false},
		            	    {name:'PI_COMPANY_ID', fixed: true, width:120},
		            	    {name:'PI_COMPANY_ABBR_NAME', fixed: true, width:100},
...
// 		            	    {name:'HANDLECHARGE', fixed:true,width: 150,align:'right',formatter:'currency', formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 2}},
		            	    {name:'TYPE_AUZ', fixed: true, width:120},
		            	    {name:'TYPE_CARD', fixed: true, width:120},
		            	    {name:'TYPE_USER_ACCT', fixed: true, width:140},
						...
						beforeProcessing: function(data, status, xhr){
							for(var rowCount in data){
								...
								data[rowCount].BTN ='<button type="button" id="edit_'+rowCount+'" value="edit" onclick="edit_p(this.value,\''+data[rowCount].PI_COMPANY_ID+'\' ,\''+data[rowCount].BILL_TYPE_ID+'\' )" ><img src="./images/edit.png"/></button>';
							}
						},
...
// 						var qStr = "component=pi_company_profile_bo&method=search_toJson&serchStrs="+encodeURI($("#serchStrs").val())+"&"+encodeURI($("#formID").find("[name!='serchStrs']").serialize());
						var qStr = "component=pi_company_profile_bo&method=search_toJson&"+$("#formID").serialize()+"&serchStrs="+encodeURI($("#serchStrs").val());
			...
			function edit_p(str, pi_company_id , bill_type_id){
				var tmp={};
