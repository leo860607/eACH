				...
				<html:form styleId="formID" action="/fee_code_nw" method="POST">
					...
					<table>
						<tr>
							...
							<td><html:text styleId="FEE_ID" property="FEE_ID" ></html:text>
							<td class="header necessary" style="width: 15%"><span>收費類型</span></td>
							<td>級距</td>
						</tr>
						<tr>
							<!-- 							TODO 未完成  要加入檢核啟用日期必須大於營業日期 -->
							<td class="header necessary"><span>啟用日期</span></td>
							<td><html:text styleId="START_DATE" property="START_DATE"></html:text>
							</td>
						...
						<tr>
							...
							<td><html:text styleId="FEE_LVL_BEG_AMT" property="FEE_LVL_BEG_AMT" size="13" maxlength="13"></html:text></td>
							<td class="header necessary"><span>交易金額(迄)</span></td>
							<td><html:text styleId="FEE_LVL_END_AMT" property="FEE_LVL_END_AMT" size="13" maxlength="13"></html:text></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td colspan="3"><html:text styleId="HANDLECHARGE" property="HANDLECHARGE" size="9" maxlength="9" styleClass="validate[decimal[7,2]]" style="text-align: right"></html:text><span class="addpercent"></span></td>
						</tr>
						<tr>
...
<!-- 										<label class="btn" id="addmore" onclick="onPut(this.id)" disable><img src="./images/add.png"/>&nbsp;新增級距</label> -->
<%-- 									</logic:notEqual> --%>
								</logic:equal>
...
<jsp:include page="/footer.jsp"></jsp:include>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				//不知為何加上STYLECLASS不會生效,故在此加
				$("#FEE_LVL_BEG_AMT").addClass("lock");
				$("#FEE_LVL_END_AMT").addClass("lock");
				$("#FEE_LVL_BEG_AMT").attr('readonly', true);
				$("#FEE_LVL_END_AMT").attr('readonly', true);
			});
...
// 				console.log('THIS DATA FEE_UID >>'+ fee_uid);
// 				判斷啟用日期是否小於等於營業日  
...
// 					啟用日期 小於營業日 要disabled 該欄位
					if(vResult.result == "TRUE"){
						...
						$(obj).addClass('lock');
						$("#FEE_LVL_TYPE").prop("outerHTML");
						if('<bean:write name="fee_code_nw_form" property="FEE_LVL_TYPE"/>'=='1'){
							$("#FEE_LVL_TYPE").prop('outerHTML','固定');
						}else if('<bean:write name="fee_code_nw_form" property="FEE_LVL_TYPE"/>'=='2'){
							$("#FEE_LVL_TYPE").prop('outerHTML','百分比');
						}
						isDelete = false;
					}else{
			...
			function onPut(str){
				if(str == "update"){
					if(!$("#formID").validationEngine("validate") || !checkFee() || !checkAmount()){
						return;
					}
					if(isDelete){
					...
					$("#ac_key").val("update_lvl");
// 					console.log("級距更新")
					$("form").submit();
					return;
					
				}else if(str == "delete"){
					if(!confirm("是否確定刪除此級距組?")){return false;}else{$("#formID").validationEngine('detach');}
					if(isDelete){
...
// 				parseFloat($('#HANDLECHARGE').val())+
			...
			function checkAmount(){
				var start = $("#FEE_LVL_BEG_AMT").val();
				var end = $("#FEE_LVL_END_AMT").val();
				if(end <= start){
		            	...
		            	gridComplete:function (){
	 					...
	 					beforeProcessing: function(data, status, xhr){
							for(var rowCount in data){
								...
								if(feeDtno!='<bean:write name="fee_code_nw_form" property="FEE_DTNO"/>'){
...
// 				console.log("GET LVL INFO BY FUID");
